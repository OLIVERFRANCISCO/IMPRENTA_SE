...
def actualizar_material_con_ancho(id_material, nombre, cantidad, unidad, stock_minimo, precio, ancho_bobina):
    """
    Actualiza un material existente incluyendo el ancho de bobina
    R2: Soporte para rollos con anchos específicos
    """
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute(
        """UPDATE materiales 
        SET nombre_material = ?, cantidad_stock = ?, unidad_medida = ?, 
            stock_minimo = ?, precio_por_unidad = ?, ancho_bobina = ? 
        WHERE id_material = ?""",
        (nombre, cantidad, unidad, stock_minimo, precio, ancho_bobina, id_material)
    )
    conn.commit()
...
def obtener_materiales_por_tipo_y_ancho(tipo_material):
    ...
    cursor.execute("""
        SELECT 
            id_material,
            nombre_material,
            cantidad_stock as cantidad_stock,
            ancho_bobina,
            unidad_medida,
            stock_minimo,
            precio_por_unidad
        FROM materiales
        WHERE LOWER(nombre_material) LIKE LOWER(?)
        AND ancho_bobina > 0
        ORDER BY ancho_bobina ASC
    """, (f"%{tipo_material}%",))
...
def obtener_rollo_por_id(id_material):
    ...
    Args:
        id_material (int): ID del material/rollo
...
def actualizar_stock_rollo(id_material, metros_a_descontar):
    ...
    Args:
        id_material (int): ID del rollo
        metros_a_descontar (float): Metros a descontar (positivo)
    ...
    try:
        ...
        if not resultado:
            raise ValueError(f"Material con ID {id_material} no encontrado")

        stock_anterior = resultado['cantidad_stock']
        stock_nuevo = stock_anterior - metros_a_descontar
        ...
        return {
            ...
            'metros_descontados': metros_a_descontar
        }
...
def registrar_consumo_material_rollo(id_detalle, id_material, metros_usados):
    """
    Registra el consumo de material en el historial.

    R19: Registro de producción

    Args:
        id_detalle (int): ID del detalle de pedido
        id_material (int): ID del rollo usado
        metros_usados (float): Metros lineales consumidos
    ...
    cursor.execute("""
        ...
        (id_detalle, id_material, cantidad_usada)
        VALUES (?, ?, ?)
    """, (id_detalle, id_material, metros_usados))

    conn.commit()
    return cursor.lastrowid


def obtener_rollos_con_stock_critico(metros_minimos=5.0):
    """
    Obtiene todos los rollos con stock crítico.

    R11: Alerta de stock bajo

    Args:
        metros_minimos (float): Metros mínimos para considerar crítico

    Returns:
        list: Lista de rollos con stock crítico
    """
    conn = get_db()
    cursor = conn.cursor()
...
def guardar_nuevo_rollo(nombre_material, ancho_bobina, cantidad_inicial, unidad_medida="metros",
                        stock_minimo=5.0, precio_por_unidad=0.0):
    """
    Registra un nuevo rollo en el inventario.

    R2: Cada ancho de bobina es una fila individual

    Args:
        nombre_material (str): Nombre del material
        ancho_bobina (float): Ancho en metros
        cantidad_inicial (float): Metros lineales iniciales
        unidad_medida (str): Unidad de medida (default: "metros")
        stock_minimo (float): Stock mínimo recomendado
        precio_por_unidad (float): Precio por metro

    Returns:
        int: ID del nuevo rollo
    """
    conn = get_db()
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        (nombre_material, ancho_bobina, cantidad_stock, unidad_medida, stock_minimo, precio_por_unidad)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (nombre_material, ancho_bobina, cantidad_inicial, unidad_medida, stock_minimo, precio_por_unidad))
